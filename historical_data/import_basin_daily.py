...
For each basin, same file is inserted into both 7days and 6months tables.
  Bonwr_Daily.csv     → basin_watershed_daily_7days, basin_watershed_daily_6months
  Sbonwr_Daily.csv    → basin_subbasin_l1_daily_7days, basin_subbasin_l1_daily_6months
  Analysis_Sbswat.csv → basin_subbasin_l2_daily_7days, basin_subbasin_l2_daily_6months
"""
...
def import_watershed(cur):
    for basin, results_dir in BASINS.items():
        path = results_dir / "Bonwr_Daily.csv"
        if not path.exists():
            print(f"\n  SKIP {basin} Bonwr_Daily.csv: not found")
            continue
...
def import_subbasin_l1(cur):
    for basin, results_dir in BASINS.items():
        ...
        if not path.exists():
            print(f"\n  SKIP {basin} Sbonwr_Daily.csv: not found")
            continue
        ...
        headers, raw = read_csv(path)
        if not raw:
            print(f"\n  SKIP {basin} Sbonwr_Daily.csv: file is empty")
            continue
        rows = []
        for r in raw:
            row = dict(zip(headers, r))
...
def import_subbasin_l2(cur):
    for basin, results_dir in BASINS.items():
        ...
        if not path.exists():
            print(f"\n  SKIP {basin} Analysis_Sbswat.csv: not found")
            continue
