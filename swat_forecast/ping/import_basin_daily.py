...
def import_watershed(cur):
    ...
    for suffix, results_dir in MODELS.items():
        ...
        log.info(f"Bonwr_Daily.csv → {table}")
        if not path.exists():
...
def import_subbasin_l1(cur):
    ...
    for suffix, results_dir in MODELS.items():
        ...
        log.info(f"Sbonwr_Daily.csv → {table}")
        if not path.exists():
            log.warning(f"SKIP: {path} not found")
            continue
        ...
        headers, raw = read_csv(path)
        if not raw:
            log.warning(f"SKIP: file is empty")
            continue
        rows = []
        for r in raw:
            row = dict(zip(headers, r))
...
def import_subbasin_l2(cur):
    ...
    for suffix, results_dir in MODELS.items():
        ...
        log.info(f"Analysis_Sbswat.csv → {table}")
        if not path.exists():
            log.warning(f"SKIP: {path} not found")
            continue
...
if __name__ == "__main__":
    run([import_watershed, import_subbasin_l1, import_subbasin_l2],
        ...
        script_name="import_basin_daily")
